...
const canBid = computed(() => isAuthenticated.value && !!auction.value?.auctionId && isAuctionActive.value)
...
const loadAuctionAndBids = async () => {
  ...
  try {
    ...
    const linkedAuction = auctions.find((item) => item.lotId === lot.value?.id)
    ...
    auction.value = await auctionService.getAuctionDetails(linkedAuction.auctionId)
...
const loadLot = async () => {
  try {
    ...
    errorMessage.value = 'Failed to load listing.'
  } finally {
...
const submitBid = async () => {
  ...
  if (!canBid.value || !auction.value?.auctionId || !lot.value) {
    bidError.value = 'Bidding is currently unavailable.'
    return
  }
  ...
  try {
    ...
    bidError.value = error?.response?.data?.message ?? 'Failed to place bid.'
  } finally {
...
<template>
  <div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
    <div v-if="isLoading" class="rounded-2xl border p-6 text-sm text-foreground/70">
      Loading listing...
    </div>
    ...
    <div v-else-if="lot && !canSeeLot" class="rounded-2xl border p-6 text-sm text-foreground/70">
      This listing is not available.
    </div>

    <div v-else-if="lot" class="grid gap-8 lg:grid-cols-[1.2fr_0.8fr]">
      <section>
        <div class="overflow-hidden rounded-3xl border bg-background">
          <img :src="lot.media?.[0]?.url" :alt="lot.title" class="h-[420px] w-full object-cover" />
        </div>

        <div class="mt-4 grid grid-cols-2 gap-3 md:grid-cols-4">
          <div v-for="(image, index) in lot.media" :key="index" class="overflow-hidden rounded-2xl border">
            <img :src="image.url" :alt="`${lot.title} ${index + 1}`" class="h-24 w-full object-cover" />
          </div>
        </div>
      </section>

      <section class="space-y-4">
        <div class="rounded-3xl border bg-background p-6">
          <p class="text-xs uppercase tracking-[0.2em] text-foreground/50">{{ lot.brand }}</p>
          <h1 class="mt-2 text-3xl font-semibold">{{ lot.title }}</h1>
          <p class="mt-4 text-sm text-foreground/70">{{ lot.description }}</p>
        ...
        <div class="rounded-3xl border bg-background p-6">
          ...
          <div v-if="bidSuccess" class="mt-3 rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-3 text-sm text-emerald-300">
            {{ bidSuccess }}
          </div>
