...
<template>
<header class="sticky top-0 z-50 border-b border bg-background/95 backdrop-blur-md">
  <Container>
    <div class="flex min-h-76px items-center justify-between gap-4 py-3">
        ...
        <div class="w-full max-w-2xl">
          <div class="relative">
            <Input
              ...
              class="h-12 rounded-full border border-input bg-muted pl-11 pr-24 text-sm text-foreground placeholder:text-muted-foreground focus-visible:border-ring focus-visible:ring-0"
            />
            ...
            <button
              @click="handleSearch"
              class="absolute right-1.5 top-1/2 -translate-y-1/2 rounded-full bg-primary px-4 py-2 text-sm font-medium text-primary-foreground transition hover:opacity-90"
            >
              Search
            </button>
          </div>
      ...
      <div class="flex shrink-0 items-center gap-2 sm:gap-3">
        <RouterLink
          to="/login"
          class="hidden rounded-full border px-4 py-2 text-sm font-medium text-foreground transition hover:bg-accent hover:text-accent-foreground sm:inline-flex"
        >
          Sign in
        </RouterLink>
